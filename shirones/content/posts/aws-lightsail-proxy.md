...
今天领到一台免费的 AWS Lightsail，也就是光帆。落在俄亥俄，区域 us-east-2：2 核 CPU、945MB 内存、40GB 磁盘，系统是 Debian 12，带 IPv6。

## 代理

代理是手搓的。主路用 Xray 26.3.27：VLESS + Reality + XTLS Vision，另外开了后量子的 ML-DSA-65 签名校验，指纹设成随机（randomized）。
...
Lightsail 默认给的是动态公网地址。重启之后地址变了，从一段换到另一段。想稳定用，还是绑一个静态地址。

从大陆走普通线路，延迟大概 200-270 毫秒，所以上面补了 Hysteria2。

从洛杉矶的一台机器测下来，Hysteria2 下载大概每秒 15MB。
